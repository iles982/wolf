...
void    generate_3d_projection_1(t_wolf * sdl, t_ray ray[1280], t_player *player, t_projection *p)
{
    p->distance_perp = ray[p->i].distance * cos(ray[p->i].ray_angle - player->rotat_angle);
    ...
    p->wall_bot_pix = p->wall_bot_pix > WINDOW_HEIGHT ? WINDOW_HEIGHT : p->wall_bot_pix;
...
void	generate_3d_projection(t_wolf * sdl, t_ray ray[1280], t_player *player)
{
	...
	while (p.i < sdl->num_rays)
	{
		...
		{
			p.distance_from_top = p.y + (p.wall_strip_height / 2) - (WINDOW_HEIGHT / 2);
			p.text_offset_y = p.distance_from_top * ((float)TEXTURE_HEIGHT / p.wall_strip_height);
			Uint32 texel_color = sdl->textures[text_num][(TEXTURE_HEIGHT * p.text_offset_y) + p.text_offset_x];
